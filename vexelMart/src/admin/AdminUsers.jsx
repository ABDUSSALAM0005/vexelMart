...
const makeAdminHandler = async (id) => {
    if (window.confirm("Are you sure you want to make this user an Admin?")) {
      try {
        ...
        await api.put(`/user/${id}`, { isAdmin: true });
        
        // 2. Show success message
        toast.success("User promoted to Admin");

        // 3. IMPORTANT: Refresh the table data immediately
        fetchUsers(); 

      } catch (error) {
        toast.error(
          error.response?.data?.message || "Error making user Admin"
        );
      }
    }
  };
            ...
            <tbody className="divide-y divide-gray-700">
              {users.map((user) => (
                <tr key={user._id} className="hover:bg-gray-700/50 transition">
                  ...
                  <td className="px-6 py-4 text-blue-400 underline decoration-blue-400/30">
                    {user.email}
                  </td>
                  ...
                  <td className="px-6 py-4 text-right flex justify-end gap-3">
                    ...
                    {user.isAdmin === false && (
                        <button
                        className="mx-2 px-4 py-2 bg-primary text-white rounded-xl"
